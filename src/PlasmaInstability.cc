...
double PlasmaInstabilityBroderick2012::energyLoss(crpropa::Candidate *candidate) const {
	...
	double nMedium = mediumDensity->getDensity(pos - pos0);
	double nBeam = flowProperties->getDensity(pos - pos0);
...
double PlasmaInstabilityBroderick2012B::energyLoss(crpropa::Candidate *candidate) const {
	...
	crpropa::Vector3d pos = candidate->current.getPosition();
	crpropa::Vector3d pos0 = candidate->source.getPosition(); 
	double lf0 = candidate->current.getLorentzFactor();
	double lf = flowProperties->getMeanLorentzFactor(pos, z, lf0);
	double ilf = flowProperties->getMeanInverseLorentzFactor(pos, z, lf0);
...
double PlasmaInstabilityVafin2018::energyLoss(crpropa::Candidate *candidate) const {
	...
	double nMedium = mediumDensity->getDensity(pos - pos0, z);
	double nBeam = flowProperties->getDensity(pos - pos0. z);
	double L = beamDensityToLuminosity(nBeam, E, z);
	double T = mediumTemperature->getTemperature(pos);
...
double PlasmaInstabilityVafin2018B::energyLoss(crpropa::Candidate *candidate) const {
	...
	double wp = plasmaFrequency(nMedium, id);
	double wmax = maximumLinearGrowthFrequency(nBeam, nMedium, ilf, id);
...
double PlasmaInstabilityVafin2018C::energyLoss(crpropa::Candidate *candidate) const {
	...
	double z = candidate->getRedshift();
	double E = candidate->current.getEnergy() * (1 + z); // multiply by (1 + z) for E(z)
	double lf0 = candidate->current.getLorentzFactor() * (1 + z);
	crpropa::Vector3d pos = candidate->current.getPosition();
	crpropa::Vector3d pos0 = candidate->source.getPosition();
	double lf = flowProperties->getMeanLorentzFactor(pos, z, lf0);
	double ilf = flowProperties->getMeanInverseLorentzFactor(pos, z, lf0);
...
double PlasmaInstabilitySchlickeiser2012B::energyLoss(crpropa::Candidate *candidate) const {
	...
	crpropa::Vector3d pos0 = candidate->source.getPosition();
	double lf0 = candidate->current.getLorentzFactor() * (1 + z);
	double lf = flowProperties->getMeanLorentzFactor(pos, 0, lf0);
	double ilf = flowProperties->getMeanInverseLorentzFactor(pos, 0, lf0);
	double nMedium = mediumDensity->getDensity(pos, z);
	double nBeam = flowProperties->getDensity(pos, z);
	double T = mediumTemperature->getTemperature(pos, z);
