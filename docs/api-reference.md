...
Header: `include/grplinst/PlasmaInstability.h`. 
The abstract CRPropa module that applies the effective cooling.
...
- `getLimit`: getter corresponding to what was set in `setLimit`;
- `getFlowProperties`: the properties of the beam, including luminosity and beam density;
- `getMediumDensity`: the density of the medium at a given position and redshift;
- `getMediumTemperature`: the temperature of the medium at a given position and redshift;
- `getEfficiencyFactor`: get the parameter $$\eta$$, which indicates how efficient is the cooling due to the instability.
...
6. limits the next step to 
    $$\quad \ell \, E \left( \dfrac{\mathrm{d}E}{\mathrm{d}x} \right)^{-1} $$,   
...
```
Returns the characteristic cooling time `τ` **in seconds**. This is the only method a new prescription must provide. 
The concrete subclasses are listed in [Models](models.html).



## Flow {#flow}

Header: `include/grplinst/Flow.h`. 
Abstract base describing the pair-beam density.
...
- `setLuminosity` / `getLuminosity`: source luminosity (in Watts); defaults to 1.
- `setPairProduction` / `addPairProduction`: optionally attach CRPropa `EMPairProduction` modules so that the beam density is computed from their *actual rates* rather than based on analytical approximations.
- `setInverseCompton` / `addInverseCompton`: same as the one for pair production, but for CRPropa's `EMInverseComptonScattering` modules.


### Pre-implemented types

- `FlowHomogeneous`: Constant-luminosity beam whose density. 
...
- `FlowJet1D`: A one-dimensional beam profile `n_b(r)` tabulated along the line of sight.
```cpp
FlowJet1D(const std::vector<double>& distances, const std::vector<double>& beamDensity, double luminosity = 1, crpropa::Vector3d centre = {0, 0, 0}, bool interpolateLog = true);
```
...
*Accessors*: 
`setDistanceProfile`, `setDensityProfile`, `setInterpolateLog`, `getDistanceProfile`, `getDensityProfile`.
...
```
- *`plasmaFrequency(n, id)`*:  the plasma frequency for a given medium density $$n$$, given by 
  $$\quad \omega_\mathrm{pl} = \sqrt{\dfrac{n q^2}{m \varepsilon_0}}$$,  
  in Hz. For electrons (`id = 11`) $$q = e$$ and $$m = m_e$$; nuclei use their charge number and nuclear mass.
- *`maximumLinearGrowthFrequency(n_b, n, γ, id)`*: the maximum linear growth rate, given by: \
  $$\quad \omega_\mathrm{pl}(n) \, \dfrac{n_\mathrm{b}}{n} \, \dfrac{1}{\gamma}$$. \
Neglects magnetic fields and assumes an angular spread:
  $$\Delta\theta = \langle \gamma^{-1} \rangle$$. 
