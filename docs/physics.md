...
$$\qquad \gamma + \gamma_\mathrm{bg} \;\rightarrow\; e^{+} + e^{-} .$$ \
...
$$\qquad e^{\pm} + \gamma_\mathrm{bg} \;\rightarrow\; e^{\pm} + \gamma$$,  
producing secondary gamma rays at GeV energies.  Those can pair-produce again, and so on: the result is an *electromagnetic cascade* that reprocesses TeV power down to GeV energies.

The observable consequences are: a "pair echo", a spatially extended "pair halo", and the reprocessed spectrum. They are all sensitive to the *intergalactic magnetic field* (IGMF), due to the action of the field on the pairs.  This makes blazar cascades one of the few probes of magnetic fields in cosmic voids. 
CRPropa's `EMPairProduction` and `EMInverseComptonScattering` modules implement exactly this chain.


## 2. The plasma-instability effect
...
Only *electrons and positrons* are affected ($$|\mathrm{id}| = 11$$); photons are ignored by the code. 

Each model provides a single quantity, a *characteristic cooling time* $$\tau(E, n_b, n, T, z)$$, which the timescale on which a pair of energy $$E$$ loses its energy to the instability. 
The base module turns this into an energy-loss *length* and applies it during propagation,\
$$\qquad \dfrac{\mathrm{d}E}{\mathrm{d}x} = \eta \, \dfrac{E}{c\,\tau}$$, \
...
A `Flow` object stores properties related to the beam, including the beam luminosity (for normalisation), and the density, through `getDensity(energy, position, redshift)`.
...
It assumes the source injects all of its luminosity $$L$$ into the beam and that IC cooling sets the pair lifetime, giving an *upper limit* of sorts on the beam density (their eq. 7):\
 $$\qquad n_b(E, z) = \dfrac{L}{2\pi \, \lambda_{\gamma\gamma}^{3} \, \Gamma_\mathrm{IC}} \, \dfrac{1}{E}$$,\
wherein $$\lambda_{\gamma\gamma}$$ is the *pair-production mean free path* of the parent photon. 
If no CRPropa pair-production module is attached, the code uses the analytic approximation\
$$\qquad \lambda_{\gamma\gamma} \approx 35\ \mathrm{Mpc} \, \dfrac{0.5\,\mathrm{TeV}}{E} \, \left(\dfrac{1+z}{2}\right)^{-4.5}$$,\
the factor $$0.5$$ reflects that a pair member carries about half the parent photon energy. Here $$\Gamma_\mathrm{IC}$$ is the *inverse-Compton loss rate*, which in the Thomson regime is given by,\
$$\qquad \Gamma_\mathrm{IC} = \dfrac{4\sigma_\text{T} \, c}{3} \, \dfrac{E}{m_e c^2} \, \dfrac{u_\mathrm{CMB}}{m_e c^2} \, (1+z)^4$$.
...
Because the beam density enters the cooling time strongly (see the exponents in [Models](models.html)), the choice of `Flow` model is often the dominant systematic. 

`FlowJet1D` lets you impose a tabulated $$n_b(r)$$ profile instead, and you can sub-class `Flow` in Python for anything else.


### 4.2 The ambient density (`MediumDensity`)

$$n$$ is the number density of the background IGM plasma. 
It sets the local plasma frequency \
$$\qquad \omega_\mathrm{pl} = \sqrt{\dfrac{n e^2}{m_e \varepsilon_0}}\,$$, \
which controls how the medium responds to the beam. `MediumDensityHomogeneous` returns a constant comoving value scaled to the proper density,\
$$\qquad n(z) = n_0 (1+z)^3$$.
...
Models such as Schlickeiser et al. (2012) and Vafin (2018) depend on $$T$$ explicitly. 
`MediumTemperatureHomogeneous` returns \
$$\qquad T(z) = T_0 (1+z)$$, \
and the base class can convert temperature into a thermal velocity $$v = \sqrt{k_B T / m}$$ through `getVelocity`.
...
| Beam density | $$n_b(z) = n_b^\mathrm{com}\,(1+z)^3$$ |
| Medium temperature | $$T(z) = T_0\,(1+z)$$ |

The step length is likewise de-redshifted, \
$$\qquad \mathrm{d}x = \dfrac{\Delta x}{1 + z}$$, \
before the loss is applied. You normally do not apply these factors yourself; instead, you specify present-day comoving values and the module handles the scaling.
...
The cooling times are analytic fits or order-of-magnitude estimates from the literature, each derived under its own assumptions (monochromatic vs. spread beams, cold vs. warm plasma, linear vs. saturated growth). A first-principles answer needs particle-in-cell (PIC) simulations.
- **The prescriptions disagree.** Different models can give cooling times that differ by orders of magnitude for the same inputs. That spread is a feature to be explored, not a bug; remember that `grplinst`'s goal is to compare models rather than trusting a single one.
- **The beam-density estimate is an upper limit.** `FlowHomogeneous` assumes 100% of the luminosity goes into the beam and IC-limited lifetimes; realistic densities are lower, which increases the cooling time.
- **Non-linear saturation is not modelled** except when it is embedded directly into the prescription model.
